...
pubnub = Pubnub(publish_key='pub-c-93c6c384-e1a0-412f-87cf-e626aaab6a00', \
	subscribe_key='sub-c-8ec9d89e-e4aa-11e5-a4f2-0619f8945a4f', 'sec-c-YzQyMTU3NmYtMDNhMS00MzM5LTg3MTgtZjA2N2U0N2IyNGY3', \
...
def sub_callback(message, channel):
	...
	if "CV_Data" in message.keys():
		logging.basicConfig(filename='logs/hydrobase_cv.log',level=logging.DEBUG)
		logging.info(message)
		...
		# insert in display
		logging.basicConfig(filename='logs/hydrobase_display.log',level=logging.DEBUG)
		logging.info(message_array)
		db.display.insert_many(message_array)

		# insert in data db
		if int(time_diff.hours) >= 4:
			logging.basicConfig(filename='logs/hydrobase_data.log',level=logging.DEBUG)
			logging.info(message_array)
			db.display.insert_many(message_array
